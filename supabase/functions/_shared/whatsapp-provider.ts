...
export const ZAVU_TEMPLATE_VARIABLES: Record<MessageCategory, string[]> = {
  ...
  // Template: gk_expired_membership_reminder — {{1}}=name, {{2}}=days_expired, {{3}}=expiry_date, {{4}}=branch_name
  expired_reminder:    ["name", "days_expired", "expiry_date", "branch_name"],
    ...
    if (!config?.zavu_api_key_encrypted || !config?.zavu_api_key_iv || !encryptionKey) {
      ...
      if (!templateId) {
        result = {
          success: false,
          provider: "zavu",
          error: `zavu_template_not_configured:${effectiveCategory}`,
        };
      } else {
        try {
