            ...
            <Card className="border border-border/40 shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden relative">
              ...
              <CardHeader className="p-4 lg:p-6 pb-3 lg:pb-4 pl-5 lg:pl-7">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-10 h-10 lg:w-11 lg:h-11 rounded-xl bg-primary/10 text-primary shadow-sm">
                    <CurrencyRupeeIcon className="w-5 h-5 lg:w-5.5 lg:h-5.5" />
                  </div>
                  <div>
                    <CardTitle className="text-base lg:text-xl font-bold tracking-tight">Monthly Packages</CardTitle>
              ...
              <CardContent className="space-y-3 lg:space-y-4 p-4 lg:p-6 pt-0 lg:pt-0 pl-5 lg:pl-7">
                ...
                <Button onClick={handleAddMonthlyPackage} disabled={isAddingMonthly} className="gap-1.5 lg:gap-2 h-9 lg:h-10 text-xs lg:text-sm rounded-xl active:scale-[0.97] transition-all duration-200 shadow-sm">
                ...
                {monthlyPackages.length > 0 && (
                  <div className="space-y-2 lg:space-y-2.5 pt-4 lg:pt-5 border-t border-border/40">
                    <p className="text-[10px] lg:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1">Active Plans ({monthlyPackages.filter(p => p.is_active).length} of {monthlyPackages.length})</p>
                    {monthlyPackages.map((pkg, index) => (
                      ...
                      >
                        {editingMonthlyId === pkg.id ? (
                          ...
                          <>
                            <div className="flex items-center justify-center w-10 h-10 lg:w-12 lg:h-12 rounded-xl bg-primary/10 text-primary text-base lg:text-lg font-black tabular-nums shrink-0 shadow-sm">
                              {pkg.months}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="font-bold text-sm lg:text-base tracking-tight">{pkg.months} {pkg.months === 1 ? "Month" : "Months"}</p>
                              <div className="flex items-center gap-1.5 mt-0.5">
                                ...
                                {pkg.joining_fee === 0 && (
                                  <span className="text-xs text-muted-foreground">+ ₹0 joining fee</span>
                                )}
                              </div>
                            </div>
                            {!pkg.is_active && (
                            ...
                            <div className="flex items-center gap-2 lg:gap-3 shrink-0">
                              ...
                              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all duration-200">
                                <Button variant="ghost" size="icon" onClick={() => handleEditMonthlyPackage(pkg)} className="h-8 w-8 lg:h-9 lg:w-9 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted active:scale-90 transition-all">
